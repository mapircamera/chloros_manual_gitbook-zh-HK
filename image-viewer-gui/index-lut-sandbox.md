...
指数/LUT沙盒是Chloros图像查看器中的交互式工作区，支持实时进行多光谱指数计算与彩色可视化实验。这款强大工具可帮助您测试不同指数、优化数值范围，并创建可直接发布的可视化效果，无需重新处理整个数据集。
...
完整指数公式文档详见[多光谱指数公式](../project-settings/multispectral-index-formulas.md)。
...
{% hint style=&quot;success&quot; %}
**可视化效能**：为灰度索引图像应用彩色LUT后，可视化模式、异常及关注区域的识别效率将显著提升。
{% endhint %}
...
2. 将鼠标悬停其上查看可用渐变预设
3. 选择所需渐变
4. 勾选索引框后，图像将**即时更新**为新颜色

{% 提示 style=&quot;success&quot; %}
**最佳实践**：对于植被指数（如NDVI），Red-Yellow-Green渐变色最直观，因其符合自然色彩关联（绿色=健康，黄色=中等，红色=受胁）。
{% endhint %}
...
* **较少类数**（3-5）： 形成鲜明分区，简化分类，便于区分类别
* **中等类别数** (6-10)：平衡方案，适用于多数场景
* **高类别数** (15-20)：平滑过渡，精细变化，呈现照片级效果

**适用场景：**

* **低类别数 (3-5)**：演示文稿、分类地图、简易报告
...
* **在范围内**的像素呈现**色彩渐变**
* **应用场景**：细微高亮处理，在保持上下文的同时突出重点区域
* **示例**：以彩色高亮显示受胁植被（NDVI 0.3-0.5），健康区域以灰色显示
...
4. **检查像素值** - 移动光标观察数值范围
5. **调整最小/最大值** - 缩小范围聚焦植被（如0.2至0.9）
6. **选择裁剪模式** - 尝试&quot;原始背景&quot;以保留上下文
7. **精细调整色彩** - 根据需要定制渐变以突出特定区域
8. **最终确认设置** - 记录参数并复制至项目设置用于导出处理
...
检查像素值后，相应调整LUT的最小/最大值：

**示例场景：**

* **观测结果**：土壤值 = 0.05-0.25，受胁值 = 0.25-0.50，健康值 = 0.50-0.85
* **目标**：仅可视化植物健康状态（排除土壤）
