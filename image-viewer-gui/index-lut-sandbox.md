...
* **输出**：RGB 颜色（例如亮绿色）
* **用途**：使数据模式更易于观察和解读**灰度LUT与彩色LUT对比：**
...
2. 悬停鼠标查看可用渐变预设
3. 选择所需渐变
4. 勾选索引框后图像将**即时更新**新色彩

{%提示 style=&quot;success&quot; %}
**最佳实践**： 对于植被指数（如NDVI），**Red-黄-Green**渐变最直观，因其符合自然色彩认知（绿=健康，黄=中等，红=受胁）。
{% endhint %}

### 调整颜色类别

**类别控制**决定渐变中显示的离散色阶数量：**类别数量选项：*** **2-5类**：宽泛分类，区域分明
* **6-10类**： 均衡，适于分类
* **11-20级**：平滑渐变，连续视觉效果
* **20+级**：近连续状态，最大平滑度**调整方法：**

1. 在LUT面板中，定位**渐变条下方色块区域**

2. 通过+按钮增加色阶数量
3. 双击色块可减少类别数量
4. 渐变效果将**实时**更新至图像**可视化效果：*** **少类别**（3-5）：形成鲜明区域，简化分类，便于区分类别
* **中等类别**（6-10）：平衡方案，适用于多数场景
* **多类别**（15-20）：平滑过渡，精细差异，呈现照片级效果**适用场景：*** **少类别（3-5）**：演示幻灯片、分类地图、简易报告
* **中等类别（6-10）**：常规分析、均衡细节、标准报告
* **多类（15-20）**：科学分析、精细检测、出版级输出

### 精细调整数值范围

**数值范围控制**决定渐变色中各指数值对应的颜色：**LUT面板中的范围控制：*** **最小值**：色阶下限
* **最大值**：色彩标尺上限
* **中间值**：自动分配于最小值与最大值之间（基于类别数量）
...
6. 可视化效果**即时更新**{%提示 style=&quot;info&quot; %}**自动缩放**：初次应用查找表时，Chloros会自动将最小/最大值设为图像实际数据范围。您可随后缩小该范围以聚焦特定关注值域。
{% endhint %}

**NDVI 范围调整示例：*** **全范围**：`-1.0` 至 `1.0`（显示所有可能值）
* **植被聚焦**：`0.2` 至 `0.9` （排除裸土与水体）
* **仅健康植被**：`0.5` 至 `0.9`（仅突出显示旺盛植被）
* **胁迫检测**：`0.2` 至 `0.5` （重点标注问题区域）
* **自定义范围**：根据观测像素值调整**为何调整范围？*** **增强**目标区域的**对比度*** **排除无关值**（如水体、裸露土壤）
* **标准化**多幅图像或不同时段的**可视化效果*** **突出显示**窄值域内的**细微差异**
...
* 像素值**低于下限**→ 显示为渐变色**首色**（如红色）
* 像素值**高于上限**→ 显示为渐变色**末色**（如绿色）
* **应用场景**：突出极端值，通过饱和色显示完整数据范围
* **示例**：NDVI值低于0.2均显示为红色，高于0.9均显示为绿色

#### 2. 透明背景

* **超出范围**的像素变为**完全透明*** 仅**范围内的像素**显示颜色渐变
* **应用场景**：GIS叠加层，隔离特定数值范围，仅突出显示关注区域
* **示例**：仅显示NDVI 0.4-0.7区间为彩色，其余透明

{% hint style=&quot;warning&quot; %}
**透明度限制**：透明像素在查看器中将显示为背景色。处理过程中导出时，PNG格式保留透明度，JPG格式则不保留。
{% endhint %}

#### 3. 索引背景

* **超出范围**的像素以**灰度**显示 （显示原始指数值）
* **范围内**像素呈现**色彩渐变*** **应用场景**：细微高亮处理，在保持上下文的同时突出重点区域
* **示例**：以彩色高亮显示受胁植被（NDVI 0.3-0.5），健康区域以灰色呈现

#### 4. 原生背景

* **超出范围**像素显示**原始多光谱图像*** **在范围内的**像素呈现**色彩渐变*** **应用场景**：最直观模式——融合自然图像背景与分析性色彩叠加
...
| **索引背景**       | 微调强调，保持数据关联   | 范围着色，超出灰阶 |
| **原始背景**     | 报告演示，直观分析         | 范围着色，超出照片 |

### 创建自定义LUT颜色

为实现可视化效果的完全掌控，您可通过编辑单个颜色停点创建**自定义渐变色**。**创建自定义渐变步骤：**
...
* **品牌色**：匹配组织报告的配色方案
* **色盲友好**：使用橙蓝或紫黄配色组合
* **打印优化**：选择适用于彩色与灰度打印的色彩
* **多阈值**：在特定数值阈值处使用不同颜色进行分类

{% 提示 style=&quot;info&quot; %}
**保存自定义渐变**：可保存并复用自定义渐变方案。点击LUT面板中的保存图标，即可将自定义配色方案留存供后续使用。
{% endhint %}
...
* **变更类别** → 梯度平滑度即时更新
* **修改裁剪** → 背景显示即时变化
* **编辑颜色** → 自定义梯度即时生效**无需&quot;应用&quot;按钮** - 所有变更均实时交互！
...
5. **记录**不同特征的数值范围：
   ...
   * **水体**（若存在）：例如 NDVI -0.05 至 0.10**利用像素值设定LUT范围：**检查像素值后，相应调整LUT最小/最大值：**示例场景：*** **观测结果**：土壤值 = 0.05-0.25，受胁值 = 0.25-0.50，健康值 = 0.50-0.85
* **目标**：仅可视化植物健康状况 （排除土壤）
...
{%提示 style=&quot;info&quot; %}
**动态范围**：不同作物、季节及生长阶段具有不同数值范围。设置LUT范围前务必检查特定数据集的像素值。
{% endhint %}
