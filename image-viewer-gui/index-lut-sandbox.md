...
在Chloros中，索引可于项目处理阶段应用。要确定导出时需采用的索引与LUT设置，最便捷的方式是使用图像查看器沙盒。
...
1. 在文件浏览器图像网格中点击图像，该图像将在**图像查看器** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> 标签页中打开
...
{% hint style=&quot;success&quot; %}
**可视化效能**：为灰度索引图像应用彩色LUT后，可大幅提升模式识别、异常检测及目标区域定位的直观效率。
{% endhint %}
...
{%提示 style=&quot;success&quot; %}
**最佳实践**：对于植被指数（如NDVI），Red-Yellow-Green渐变色最直观，因其符合自然色彩认知（绿色=健康，黄色=中等，红色=受胁）。
{% endhint %}
...
* **较少类别**（3-5类）：形成鲜明区域，简化分类，便于区分类别
* **中等类别数** (6-10)：平衡方案，适用于多数场景
* **较多类别数** (15-20)：平滑过渡，精细差异，逼真照片质感

**适用场景：**

* **少类别数 (3-5)**：演示文稿、分类地图、简易报告
* **中等类别（6-10）**：通用分析、均衡细节、标准报告
* **大量类别（15-20）**：科学分析、精细检测、出版级输出
...
{%提示 style=&quot;info&quot; %}
**自动缩放**：初次应用LUT时，Chloros会自动将最小/最大值设为图像实际数据范围。您可随后缩小该范围以聚焦特定关注值域。
{% endhint %}
...
* **示例**：仅显示NDVI 0.4-0.7区间彩色，其余透明

{% hint style=&quot;warning&quot; %}
**透明度限制**：透明像素在查看器中将呈现为背景色。 处理过程中导出时，PNG格式保留透明度，JPG格式则不保留。
{% endhint %}
...
为实现可视化效果的完全掌控，您可通过编辑单个颜色停点创建**自定义渐变色**。
...
1. **选择指数层**（如RAW（反射率））
2. **应用指数** - 选择相机滤镜与指数公式，将彩色圆圈拖拽至指数公式中相应位置
3. **应用LUT梯度** - 从Red-Yellow-Green预设开始
4. **检查像素值** - 移动光标观察数值范围
5. **调整最小/最大值** - 缩小范围聚焦植被（例如0.2至0.9）
6. **选择裁剪模式** - 尝试&quot;原始背景&quot;模式获取上下文
7. **精细调整色彩** - 根据特定强调需求定制渐变曲线
...
1. 当图像勾选索引值框或同时勾选索引值与LUT框时，像素值将显示
2. 将光标悬停于图像不同区域
3. 观察悬停时图例中显示的像素值
4. 放大查看带浮动数值的高亮单个像素
5. **记录**不同特征的数值范围：
...
* **观测结果**：土壤值=0.05-0.25，胁迫=0.25-0.50，健康=0.50-0.85
* **目标**：仅可视化植物健康状态（排除土壤）
* **LUT设置**：最小值=`0.25`，最大值=`0.85`
* **裁剪**：&quot;原始背景&quot;模式以自然色呈现土壤
* **结果**：色彩渐变仅作用于植被，土壤显示原始图像

{% hint style=&quot;info&quot; %}
**动态范围**：不同作物、季节及生长阶段将呈现不同数值区间。 设置LUT范围前，务必检查特定数据集的像素值。
{% endhint %}
