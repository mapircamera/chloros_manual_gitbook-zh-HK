...
* **少类别**（3-5）：形成鲜明区域，简化分类，便于区分类别
* **中等类别**（6-10）：平衡方案，适用于多数场景
* **多类别**（15-20）：平滑过渡，精细变化，逼真照片质感

**适用场景：**

* **少类别（3-5）**：演示幻灯片、分类地图、简易报告
* **中类别（6-10）**：常规分析、均衡细节、标准报告
* **多类别（15-20）**： 科学分析、精细检测、出版级输出
...
{%提示 style=&quot;info&quot; %}
**自动缩放**：初次应用LUT时，Chloros会自动将最小/最大值设为图像实际数据范围。随后可缩小该范围以聚焦特定目标值区间。
{% endhint %}
...
| **透明背景**       | GIS叠加层，特定范围隔离       | 范围着色，范围外空白 |
| **指数背景**       | 微调强调，保持数据关联性       | 范围着色，范围外灰色 |
| **原始背景**       | 报告演示，直观分析支持       | 范围着色，范围外照片 |
...
{% hint style=&quot;info&quot; %}
**动态范围**：不同作物、季节及生长阶段具有不同数值范围。设置LUT范围前务必核查特定数据集的像素值。
{% endhint %}
