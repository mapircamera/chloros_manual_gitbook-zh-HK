...
在Chloros图像查看器中，通过&quot;图像图层&quot;下拉菜单可快速切换同一图像的不同版本——从原始采集图像到处理后的反射率输出图及计算得到的指数图。
...
* **统一工作流**：选定单一图层（如NDVI）完成全数据集审阅后再切换
* **快速比对**：在原始图与反射率图间切换以验证处理质量

### 性能考量

* **JPG加载最快**：适用于快速浏览大量图像
* **RAW图层加载较慢**：具备更高分辨率与位深度
* **索引图层**：加载速度与反射率图层相当
* **首次加载最慢**：同图层后续查看将缓存加速
