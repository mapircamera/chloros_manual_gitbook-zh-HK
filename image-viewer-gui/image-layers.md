...
在Chloros图像查看器中，&quot;图像图层&quot;下拉菜单可让您快速在同一图像的不同版本间切换——从原始采集图像到处理后的反射率输出，再到计算得出的指数图像。
...
* 计算后的植被指数图像（本例为NDVI）
* 指数名称根据处理过程中配置的指数类型动态变化
* 示例：原始数据（NDVI指数）、原始数据（NDRE指数）、原始数据（GNDVI指数）等
* 显示指数计算结果的单波段灰度图像
* 项目设置中配置的每个指数对应一层
...
* 若当前查看&quot;RAW (反射率)&quot;，则下一张图像显示&quot;RAW (反射率)&quot;（若存在）
* 若当前查看&quot;RAW (NDVI 索引)&quot;，则下一张图像显示&quot;RAW (NDVI 索引)&quot;（若存在）
* 若不存在相同图层，则默认显示JPG格式
...
1. 打开首张处理图像
2. 从下拉菜单选择**原始数据（XPROTX索引）**
3. 使用→方向键切换至下一张图像
4. XPROTX图层自动保持显示
5. 循环处理所有图像，核查XPROTX图案
6. 切换至**原始数据层（XPROTX索引）**进行比对
...
* **务必检查原始RAW层**：处理前验证源数据质量
* **对比图层**：通过切换图层验证处理结果正确性
